...
app.use((req, res, next) => {
  ...
  if (
    ...
    return res.status(403).json({ error: "Ge+�ersiz istek kayna���-." });
  next();
});
...
const requireUser = (req, res, next) =>
  req.user
    ? next()
    : res.status(401).json({ error: "Oturum a+�man�-z gerekiyor." });
const requireAdmin = (req, res, next) =>
  normalize(req.user?.role) === "admin"
    ? next()
    : res
        .status(403)
        .json({ error: "Bu i+�lem yaln�-zca Admin taraf�-ndan yap�-labilir." });
...
app.post("/api/login", (req, res) => {
  ...
  if (a.count >= 15)
    return res.status(429).json({
      error: "+�ok fazla ba+�ar�-s�-z giri+�. 15 dakika sonra tekrar deneyin.",
    });
  const s = loadState();
  if (!s)
    return res.status(503).json({
      error: "+�nce npm run import komutuyla Excel dosyas�-n�- aktar�-n.",
    });
  ...
  if (
    ...
    return res.status(401).json({
      error:
        "Kullan�-c�- ad�- veya +�ifre yanl�-+�; hesab�-n�-z�-n aktif oldu��unu kontrol edin.",
    });
  }
  attempts.delete(key);
  ...
  try {
    ...
    res
      ...
      .json({
        error:
          e.name === "TimeoutError"
            ? "PESDATA ba��lant�-s�- zaman a+��-m�-na u��rad�-. Yerel katalog kullan�-labilir."
            : e.message,
      });
  }
});
app.get("/api/matches/:id/squad", requireUser, (req, res) => {
  if (!permitted(req.user, "AYARLAR.SKOR"))
    return res.status(403).json({ error: "Skor giri+� yetkiniz yok." });
  const s = loadState(),
    m = s.matches.find((x) => x.id === Number(req.params.id));
  if (!m) return res.status(404).json({ error: "Ma+� bulunamad�-." });
...
app.post("/api/action", requireUser, (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({ error: "Ge+�ersiz i+�lem." });
  if (op === "catalog.import") {
    ...
    if (payload.cards.some((c) => !c))
      return res
        .status(400)
        .json({ error: "Arama sonu+�lar�-n�- yenileyip tekrar se+�in." });
  }
  const result = mutate(
...
app.post("/api/upload", requireUser, upload.single("image"), (req, res) => {
  if (
    ...
    return res.status(403).json({ error: "G+�rsel y+-kleme yetkiniz yok." });
  ...
  if (!ext)
    return res
      .status(400)
      .json({ error: "JPG, PNG veya WebP g+�rsel se+�in (en fazla 8 MB)." });
...
app.post("/api/restore", requireUser, requireAdmin, (req, res) => {
  ...
  if (b.format !== "efootball-v228-web" || b.version !== 1 || !b.state)
    return res.status(400).json({ error: "Ge+�erli bir web yede��i se+�in." });
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Yedekte aktif y+�netici hesab�- yok." });
  fs.writeFileSync(
  ...
  res.clearCookie("ef228", { path: "/" });
  res.json({ message: "Yedek geri y+-klendi. Yeniden giri+� yap�-n." });
});
app.get("/api/export", requireUser, (req, res, next) => {
  if (!permitted(req.user, "AYARLAR.LIG"))
    return res.status(403).json({ error: "D�-+�a aktarma yetkiniz yok." });
  const s = loadState(),
  ...
  res.status(err.status || 400).json({
    error:
      err.code === "LIMIT_FILE_SIZE"
        ? "Dosya 8 MB s�-n�-r�-n�- a+��-yor."
        : err.message || "�-+�lem tamamlanamad�-.",
  });
});
...
app.listen(port, process.env.HOST || "127.0.0.1", () =>
  console.log(
    `eFootball v228 ba���-ms�-z web uygulamas�-: http://localhost:${port}`,
  ),
);

...
export function migrateAccessData(s) {
  ...
  if (!viewer) {
    viewer = {
      id: nextId(s.roles),
      name: "İzleyici",
      description: "Salt okunur izleyici",
      system: true,
    };
    s.roles.push(viewer);
  }
  ...
  if (!leagueRole) {
    leagueRole = {
      id: nextId(s.roles),
      name: "LİG ADMİNİ",
      description: "Atandığı ligde oyuncu, kadro, skor ve istatistik yönetimi",
      system: false,
      scope: "league",
    };
    s.roles.push(leagueRole);
  }
  for (const role of s.roles)
    role.scope = isLeagueRole(role) ? "league" : "global";
  for (const code of [
    "AYARLAR.OYUNCU",
    "AYARLAR.SKOR",
    "KATALOG.GORUNTULE",
    ...
    if (
      !s.permissions.some((p) => p.role === leagueRole.name && p.code === code)
    )
      s.permissions.push({
  ...
  for (const code of READ_PERMISSIONS)
    if (!s.permissions.some((p) => p.role === viewer.name && p.code === code))
      s.permissions.push({
        id: nextId(s.permissions),
        role: viewer.name,
        code,
